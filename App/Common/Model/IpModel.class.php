...
class IpModel extends Model{
	...
	public function __construct(){
		if(!$filename) $filename = dirname(__FILE__) . '/UTFWry.dat';
     ...
     * 得到区域id
     */
    function GetAreaID($ip = ""){
        mb_internal_encoding("UTF-8");
        $areaArray = $this->getlocation($ip);
        $area = mb_substr(trim($areaArray["country"]), 0, 2, 'utf-8');
        if(S($area))  return S($area);
        $row =  M("area")->where("title like '%$area%'")->find(); //  todo 这里可以优化为memcache 存储
        if(empty($row)) return 0;
        S($area,$row["area_id"]); //设置缓存
        return $row["area_id"];
    }
}