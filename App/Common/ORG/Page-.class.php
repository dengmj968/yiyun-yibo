...
class Page {
    ...
    protected $coolPages;
    // 分页显示定制
    protected $config = array('header' => '条记录', 'prev' => '', 'next' => '', 'first' => '', 'last' => '', 'theme' => '<i class="m_page_stylist_i fl"> %totalRow% %header%  </i>%upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        if ($this->nowPage == $this->totalPages) {
            ...
            $theEnd = "<a class='last' href='" . $url . "&" . $p . "=$theEndRow' >" . $this->config['last'] . "</a><form class='fr' action='".$_SERVER['REQUEST_URI']."'><input type='text' style='height:22px;' name='p' placeholder='页数' class='m_page_stylist_input' /><input type='submit' style='height:32px;' class='m_page_stylist_go' value='GO' /></form>";
        }
        /**
        ...
        $pageStr = str_replace(
				...
				array($this->config['header'], $this->nowPage, $this->totalRows, $this->totalPages, $theFirst, $upPage, $linkPage, $downPage, $theEnd),
				$this->config['theme']
		);
        return $pageStr;
    }
