...
function updateConfigs(){
		...
		$("input[name='projectId']").each(function(){
			...
			$.ajax({
				type:'get',
				url:'/Index/updateConfs',
				data:{id:$("#insert_id").val(),pid:$('#pid').val(),web_name:$("#web_name").val(),colors:$("#colors").val(),mid:listId},
				dataType:'json',
...
function getModels(obj){
	if(obj.checked){
		var template_id = obj.getAttribute('info');
		$(".templateModels").hide();
		$(".main_"+template_id).show();
	}
	updateConfigs();
}
