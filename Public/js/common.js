...
ui = window.ui ||{
	...
	countNew:function(){
		$.getJSON( U('home/User/countNew') ,function(txt){
		    ...
		    var list = {
                             ...
                             atme:{url:U('home/user/atme'),name:"@我的博文"},
							 atme_comment:{url:U('home/user/atme_comment'),name:"@我的评论"},
	...
	getarea2:function(prefix,init_style,init_p,init_c,init_a){
		var style = (init_style)?'class="'+init_style+'"':'';
		var html = '<select name="'+prefix+'_province" '+style+'><option>省/直辖市</option></select> '+
				'<select name="'+prefix+'_city" '+style+' style="width:180px"><option value=0>不限</option></select>'+
				'<select name="'+prefix+'_area" '+style+' style="width:180px"><option value=0>不限</option></select>';
		document.write(html);
		// _PUBLIC_+'/js/area.js'
		$.getJSON(U('home/Public/getArea'), function(json){
			json = json.provinces;
				...
				if(n.id==init_p){
					 ...
					 $.each(n.citys,function(j,m){
							for(var p in m){
								...
								if(p != 'citys'){
									city+='<option rel="'+j+'" pid="'+i+'" value="'+p+'" '+cselected+'>'+m[p]+'</option>';
								}
							};
					 });
					 $("select[name='"+prefix+"_city']").html(city);
					 if(citys_num !== false){
						 $.each(n.citys[citys_num].citys,function(j,m){
								for(var p in m){
									cselected = (p==init_a)?'selected="true"':'';
									area+='<option value="'+p+'" '+cselected+'>'+m[p]+'</option>';
								};
						 });
						 $("select[name='"+prefix+"_area']").html(area);
					 }
				}
				if(n.citys[0]==init_p){
					 pselected = 'selected="true"';
			...
			$("select[name='"+prefix+"_province']").live('change',function(){
				...
				if( handle ){
					...
					$.each(t,function(j,m){
						for(var p in m){
							if(p != 'citys'){
								city+='<option rel="'+j+'" pid="'+handle+'" value='+p+'>'+m[p]+'</option>';
							}
						};
					});
				};
			...
			$("select[name='"+prefix+"_city']").live('change',function(){
				var area='<option>不限</option>';
				var handle =  $(this).find('option:selected').attr('pid');
				var handle2 =  $(this).find('option:selected').attr('rel');
				if( handle ){
					var t =  json[handle].citys[handle2].citys;
					$.each(t,function(j,m){
						for(var p in m){
							area+='<option value='+p+'>'+m[p]+'</option>';
						};
					});
				};
				$("select[name='"+prefix+"_area']").html(area);
...
var listHTML = '<li style="text-align:left"><a rel="$ID$" onclick="weibo.add_atme($(this).attr(\'rel\'))" >@$SACCOUNT$</a></li>';
...
var AutoTips = function(A){
	...
	_this.enter = function(){
		...
		weibo.add_atme(selectList.list[selectList.index].getElementsByTagName('A')[0].rel);
		_this.hide();
		return false;
	}
